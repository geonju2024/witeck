...
def main():
    ...
    # 09의 masked statistics pooling이 마지막 hand-valid 채널을 0/1 mask로 쓴다.
    X_norm[:, :, -1] = X_seq[:, :, -1]
    seq_mean[:, :, -1] = 0.0
    seq_std[:, :, -1] = 1.0
    ...
    output_dir = (
        Path(witech_root) / "derived" / "siamese_dilated_enrollment_sweep"
        if witech_root
        else Path(args.data).expanduser().resolve().parent
        / "siamese_dilated_enrollment_sweep"
    )
