"""Five-seed benchmark for the tailored D=127 hand-only SupCon model."""
...
def main() -> None:
    ...
    output_root = root / "output" / "hand_only_tailored_five_seed"
    output_root.mkdir(parents=True, exist_ok=True)
    environment = os.environ.copy()
    environment["PYTHONPYCACHEPREFIX"] = "/tmp/witeck-hand-tailored-five-pycache"

    train_jobs = []
    for seed in SEEDS:
        auth_dir = output_root / f"seed_{seed}" / "user_auth"
        auth_dir.mkdir(parents=True, exist_ok=True)
        checkpoint = auth_dir / "embedding_1dcnn_supcon.pt"
        if not checkpoint.exists():
            train_jobs.append(
                (
                    [
                        sys.executable,
                        str(root / "41_train_hand_only_supcon.py"),
                        "--data", str(data),
    ...
    if train_jobs:
        print(f"Training {len(train_jobs)} tailored hand-only models", flush=True)
        with ThreadPoolExecutor(max_workers=3) as executor:
            futures = {
                executor.submit(
                    run_logged, command, log, root, environment
                ): seed
                for command, log, seed in train_jobs
            }
            for future, seed in futures.items():
                future.result()
                print(f"seed={seed}: authentication training complete", flush=True)
    ...
    for seed in SEEDS:
        ...
        gesture_checkpoint = (
            root / "output" / "hand_only_five_seed" / f"seed_{seed}"
            / "gesture" / "gesture_embedding_1dcnn.pt"
        )
        ...
        end_to_end = parse_summary(e2e_summary)
        row = {
            "seed": seed,
        ...
        rows.append(row)
        print(
            f"seed={seed}: auth={row['auth_accuracy']:.4f} "
            f"e2e={row['e2e_accuracy']:.4f}",
            flush=True,
        )
    ...
    with (output_root / "aggregate_summary.txt").open(
        ...
        handle.write("Tailored hand-only D=127 SupCon five-seed benchmark\n")
        handle.write(f"dataset={data.resolve()}\n")
        handle.write(f"seeds={list(SEEDS)}\n")
        handle.write("architecture=hand-only-two-stream-stats-supcon\n")
    ...
    print("Tailored hand-only five-seed benchmark complete", flush=True)
