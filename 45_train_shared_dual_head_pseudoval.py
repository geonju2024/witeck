...
class SessionAwareHardNegativeBatchSampler:
    """
    한 batch 안에 다음을 의도적으로 만든다.

    Positive:
        same user + same gesture + different session

    Hard Negative:
        different user + same gesture

    여러 gesture를 한 batch에 넣기 때문에 Gesture SupCon도
    다른 gesture를 negative로 학습할 수 있다.
    """
    ...
    def __init__(
        ...
        self.session = np.asarray(session)

        self.gestures_per_batch = gestures_per_batch
        self.users_per_gesture = users_per_gesture
        ...
        if samples_per_user != 2:
            raise ValueError(
                "이번 실험에서는 samples_per_user=2를 사용하세요."
            )

        # gesture -> user -> session -> sample indices
        self.groups = {}

        for idx in range(len(self.user_y)):
            u = int(self.user_y[idx])
            g = int(self.gesture_y[idx])
            s = str(self.session[idx])

            self.groups.setdefault(g, {})
            self.groups[g].setdefault(u, {})
            self.groups[g][u].setdefault(s, [])
            self.groups[g][u][s].append(idx)

        # 같은 gesture에 대해 최소 2개 session을 가진 사용자만 사용
        self.eligible_users = {}

        for g, user_dict in self.groups.items():
            valid_users = []

            for u, session_dict in user_dict.items():
                if len(session_dict) >= 2:
                    valid_users.append(u)

            if len(valid_users) >= users_per_gesture:
                self.eligible_users[g] = valid_users

        self.eligible_gestures = sorted(
            self.eligible_users.keys()
        )

        if len(self.eligible_gestures) < gestures_per_batch:
            raise RuntimeError(
                "Session-aware sampler를 구성할 수 없습니다. "
                f"사용 가능한 gesture={self.eligible_gestures}, "
                f"필요 gesture 수={gestures_per_batch}"
            )
        ...
        if batches_per_epoch is None:
            self.batches_per_epoch = max(
                1,
                int(np.ceil(len(self.user_y) / self.batch_size)),
            )
        else:
        ...
        print(
            "[SessionAwareSampler] "
            f"eligible_gestures={self.eligible_gestures}"
        )
        print(
            "[SessionAwareSampler] "
    ...
    def __iter__(self):
        ...
        for _ in range(self.batches_per_epoch):
            ...
            selected_gestures = rng.choice(
                self.eligible_gestures,
                size=self.gestures_per_batch,
                replace=False,
            )
            ...
            for g in selected_gestures:
                candidate_users = self.eligible_users[int(g)]

                selected_users = rng.choice(
                    candidate_users,
                    size=self.users_per_gesture,
                    replace=False,
                )

                for u in selected_users:
                    session_dict = self.groups[int(g)][int(u)]

                    session_list = list(
                        session_dict.keys()
                    )

                    # 반드시 서로 다른 두 session 선택
...
def make_session_aware_loader(
    ...
    batch_sampler = SessionAwareHardNegativeBatchSampler(
        user_y=user_y,
        gesture_y=gesture_y,
        session=session,
...
def main() -> None:
    ...
    calibration_users = unseen_users

    cal_idx = np.where(
        np.isin(meta["performer"], calibration_users)
    )[0]

    gesture_embeddings, user_embeddings =       extract_dual_embeddings(
    model,
    X_norm[cal_idx],
    duration_norm[cal_idx],
    device,
    )
