...
def main() -> None:
    ...
    for target_user in target_users:
        ...
        enroll_session = target_sessions[0]

        target_gestures = sorted(
            np.unique(gesture[target_idx]).tolist()
        )
        ...
        for registered_gesture in target_gestures:
            ...
            if len(enrollment_candidates) < enrollment_per_gesture:
                print(
                    ...
                    f"(only {len(enrollment_candidates)} enrollment samples)"
                )
                continue
