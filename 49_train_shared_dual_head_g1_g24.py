...
class GestureBalancedBatchSampler:
    """Build batches with repeated samples from each gesture.

    Used for gesture-only datasets such as AH650, where reliable
    user identity labels are unavailable.
    """
...
def make_gesture_only_loader(
    ...
    """Loader for samples that have gesture labels but no identity labels."""
    ...
    sampler = GestureBalancedBatchSampler(
        gesture_y,
...
def run_gesture_only_epoch(
    ...
    """Train AH650 through the gesture embedding only.

    No user CE and no UserSupCon are computed.
    Gradients still pass through the shared backbone and gesture head.
    """
    ...
    for xb, db, gesture_y in loader:
        ...
        total_gesture_supcon += (
            float(gesture_supcon.item()) * n
        )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=str(
            project_dir
            / "dataset_G1_G24_hand_only_2902_20260923.npz"
        ),
    )
    parser.add_argument(
        "--output-dir",
    ...
    # G1~G24 unified dataset:
    # WITECK G1~G5 has identity labels.
    # AH650 G6~G24 is gesture-only.
    with np.load(args.data, allow_pickle=True) as z:
        if "identity_available" not in z.files:
            raise ValueError(
                "G1-G24 training requires identity_available metadata"
            )

        identity_available = (
            z["identity_available"]
            .astype(bool)
            .reshape(-1)
        )

    if len(identity_available) != len(X_seq):
        raise ValueError(
            "identity_available length does not match dataset"
        )
    ...
    # Identity training/validation must contain only samples
    # with trustworthy user identity.
    if not np.all(identity_available[train_idx]):
        raise RuntimeError(
            "Identity train split contains gesture-only samples"
        )

    if not np.all(identity_available[val_idx]):
        raise RuntimeError(
            "Identity validation split contains gesture-only samples"
        )

    # AH650 G6~G24:
    # train the shared backbone + Gesture Head only.
    gesture_only_idx = np.where(
        ~identity_available
    )[0]

    if len(gesture_only_idx) == 0:
        raise RuntimeError(
            "No gesture-only samples were found"
        )
    ...
    gesture_only_loader = make_gesture_only_loader(
        X_norm[gesture_only_idx],
        duration_norm[gesture_only_idx],
        gesture_y[gesture_only_idx],
        samples_per_gesture=args.samples_per_group,
        seed=args.seed + 1000,
    )
    ...
    print(
        f"identity train={len(train_idx)} "
        f"val={len(val_idx)}"
    )
    print(
        f"gesture-only train={len(gesture_only_idx)}"
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"acc={train_acc:.3f} | "
            f"AH650 GestureSupCon={gesture_only_supcon:.4f} "
            f"weighted={gesture_only_loss:.4f} | "
    ...
    torch.save(
        {
            ...
            "identity_training_samples": int(len(train_idx)),
            "identity_validation_samples": int(len(val_idx)),
            "gesture_only_training_samples": int(len(gesture_only_idx)),
            "gesture_only_identity_loss": False,
            "training_loss": (
                "user_cross_entropy"
                "+user_supcon"
                "+gesture_supcon"
